...
<template>
  ...
  <el-form v-else ref="form" v-loading="loading" :model="form" :label-width="config.labelWidth" :label-position="config.labelPosition" element-loading-text="Loading...">
    <el-row :gutter="15">
      <template v-for="(item, index) in config.formItems" :key="index">
        <el-col v-if="!hideHandle(item)" :span="item.span || 24">
          <sc-title v-if="item.component=='title'" :title="item.label" />
          <el-form-item v-else :prop="item.name" :rules="rulesHandle(item)">
            ...
            <template v-else-if="item.component=='checkbox'">
              <template v-if="item.name">
                <el-checkbox v-for="(_item, _index) in item.options.items" :key="_index" v-model="form[item.name][_item.name]" :label="_item.label" />
              </template>
              <template v-else>
                <el-checkbox v-for="(_item, _index) in item.options.items" :key="_index" v-model="form[_item.name]" :label="_item.label" />
              </template>
            </template>
            <!-- checkboxGroup -->
            ...
            <template v-else-if="item.component=='rate'">
              <el-rate v-model="form[item.name]" style="margin-top: 6px;" />
            </template>
            <!-- slider -->
            ...
            <template v-else-if="item.component=='tableselect'">
              <tableselect-render v-model="form[item.name]" :item="item" />
            </template>
            <!-- editor -->
            <template v-else-if="item.component=='editor'">
              <sc-editor v-model="form[item.name]" placeholder="请输入" :height="400" />
            </template>
            <!-- noComponent -->
            <template v-else>
