<template>
  <div v-loading="loading" class="sc-upload" element-loading-background="rgba(0, 0, 0, 0.5)" :style="style">
    ...
    <span style="display:none!important"><el-input v-model="img" /></span>
  </div>
</template>
...
const scFileSelect = defineAsyncComponent(() => import('~/components/scFileSelect.vue'))
...
export default {
  ...
  methods: {
    ...
    cropperSave () {
      const uploadFile = this.$refs.upload.uploadFiles[0].raw
    ...
    request (param) {
      ...
      const file = this.cropper ? this.cropperUploadFile : param.file
