<template>
  <div class="adminui-tags">
    <ul ref="tags">
      <li v-for="tag in tagList" :key="tag" :class="[isActive(tag)?'active':'',tag.meta.affix?'affix':'' ]" @contextmenu.prevent="openContextMenu($event, tag)">
        <router-link :to="tag">
          <span>{{ tag.meta.title }}</span>
          <el-icon v-if="!tag.meta.affix" @click.prevent.stop="closeSelectedTag(tag)"><close /></el-icon>
        </router-link>
      </li>
    </ul>
  </div>
  ...
  <transition name="el-zoom-in-top">
    <ul v-if="contextMenuVisible" id="contextmenu" :style="{left:left+'px',top:top+'px'}" class="contextmenu">
      <li @click="refreshTab()"><el-icon><refresh /></el-icon>刷新</li>
      <hr>
...
export default {
  ...
  methods: {
    ...
    refreshTab () {
      ...
      const self = this
      setTimeout(() => {
        self.$store.commit('removeKeepLive', nowTag.name)
        self.$store.commit('setRouteShow', false)
        self.$nextTick(() => {
          self.$store.commit('pushKeepLive', nowTag.name)
          self.$store.commit('setRouteShow', true)
        })
      }, 0)
    },
    // TAB 关闭
