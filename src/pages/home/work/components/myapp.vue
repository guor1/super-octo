<template>
  <div>
    ...
    <el-drawer v-model="modsDrawer" title="添加应用" :size="570" destroy-on-close>
      <el-container>
        <el-main class="nopadding">
          <el-scrollbar>
            <div class="setMods">
              <h4>我的常用 ( {{ myMods.length }} )</h4>
              <draggable v-model="myMods" tag="ul" animation="200" item-key="path" group="people">
            ...
            <div class="setMods">
              ...
              <draggable v-model="filterMods" tag="ul" animation="200" item-key="path" :sort="false" group="people">
                <template #item="{ element }">
                  <li :style="{background:element.meta.color||'#909399'}">
                    <el-icon><component :is="element.meta.icon||el-icon-menu" /></el-icon>
                    <p>{{ element.meta.title }}</p>
