        ...
        <el-card shadow="never">
          <div class="user-info">
            <div class="user-info-top">
              ...
              <h2>{{ form.name||'-' }}</h2>
              <p>{{ form.about||'无签名' }}</p>
              <ep-button type="primary" round size="default" icon="ep:collection-tag">
                Administrator
              </ep-button>
            </div>
              ...
              <el-timeline style="margin-top:20px;padding-left:10px;">
                <el-timeline-item v-for="(activity, index) in activities" :key="index" :timestamp="activity.timestamp" placement="top">
                  <div class="activity-item">
                    ...
                    <label>{{ activity.operate }}</label><el-tag v-if="activity.mod" size="small">{{ activity.mod }}</el-tag>{{ activity.describe }}
                  </div>
                </el-timeline-item>
              </el-timeline>
            </el-tab-pane>
            ...
            <el-tab-pane :label="$t('user.settings')">
              <el-form ref="form" :model="form" label-width="120px" style="margin-top:20px;">
                <el-form-item :label="$t('user.nightmode')">
                  <el-switch v-model="config.theme" active-value="dark" inactive-value="default" inline-prompt />
...
export default {
  ...
  watch: {
    ...
    'config.colorPrimary': function (val) {
